...
//            Document doc = courseRepo.getUdemyCourseById(courseId);
            Document doc = courseRepo.getUdemyCourseById(courseId);
            if (doc == null) {
                System.out.println("Course not found in the database for courseId: " + courseId);
            } else {
                System.out.println("Course found: " + doc.toJson());
            }
    ...
    public boolean unEnrollCourse(String courseId){

        String sql = "DELETE FROM registered_courses WHERE platformId = ?";
